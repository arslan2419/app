...
const Header = () => {
  ...
  return (
    <>
      ...
      {/* Backdrop Overlay */}
      <div
        className={`fixed inset-0 z-[100] md:hidden transition-all duration-700 ease-out ${isMobileMenuOpen
          ? 'opacity-100 visible'
      ...
      >
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-br from-slate-950 via-indigo-950/90 to-slate-950 backdrop-blur-2xl transition-opacity duration-700 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0'
            }`}
        />

        {/* Animated Gradient Overlay */}
        <div
          className={`absolute inset-0 bg-gradient-to-br from-indigo-600/20 via-violet-600/10 to-purple-600/20 transition-opacity duration-700 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0'
            }`}
        />
      </div>
      ...
      <div
        className={`fixed top-0 left-0 bottom-0 z-[101] md:hidden w-full max-w-[85vw] sm:max-w-md bg-gradient-to-br from-slate-900/98 via-slate-800/95 to-slate-900/98 backdrop-blur-2xl border-r border-indigo-500/30 shadow-[0_0_50px_rgba(99,102,241,0.3)] transition-all duration-700 ease-out ${isMobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'
          }`}
        onClick={(e) => e.stopPropagation()}
      >
        ...
        <div className="relative flex items-center justify-between p-6 border-b border-indigo-500/20 bg-gradient-to-r from-indigo-600/5 to-transparent">
          <div className="flex items-center gap-3">
            <img src="/zh-icon.svg" alt="ZH Solutions" className="w-14 h-auto" />
            <div className="text-2xl font-bold bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent cursor-pointer z-50 relative">
              Solutions
            </div>
          </div>
          <button
        ...
        <nav className="relative flex flex-col p-6 gap-3 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}>
          {navLinks.map((link, index) => (
            <button
              key={link.id}
              onClick={() => scrollToSection(link.id)}
              className={`group relative px-5 py-4 rounded-2xl text-left overflow-hidden transition-all duration-500 ${isMobileMenuOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'
                }`}
              style={{
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-slate-900 via-slate-900/95 to-transparent border-t border-indigo-500/20">
          <Button
            onClick={() => scrollToSection('contact')}
            className={`w-full bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 hover:from-indigo-500 hover:via-violet-500 hover:to-purple-500 text-white py-6 text-lg font-bold shadow-2xl shadow-indigo-500/40 transition-all duration-500 transform hover:scale-[1.02] hover:shadow-indigo-500/60 relative overflow-hidden group ${isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
            style={{
        ...
        <div className={`absolute top-1/3 -left-20 w-40 h-40 bg-indigo-500/20 rounded-full blur-3xl transition-opacity duration-1000 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0'
          }`} />
        <div className={`absolute bottom-1/4 -right-20 w-48 h-48 bg-violet-500/20 rounded-full blur-3xl transition-opacity duration-1000 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0'
